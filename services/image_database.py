...
class ImageDatabase:
    ...
    def __init__(self):
        ...
        self.metadata_file = os.path.join(
            self.images_dir, "image_metadata.json"
        )  # File metadata
    ...
    def extract_images_from_any_file(
        ...
        unstructured_images = self._extract_with_unstructured(
                ...
                if isinstance(element, Image):
                    try:
                        ...
                        if img_data:
                            # Save image
                            img_filename = (
                                f"unstructured_{source_filename}_img{i + 1}.png"
                            )
                            img_path = os.path.join(
                                self.images_dir, "extracted", img_filename
                            )
                ...
                for rel in doc.part.rels.values():
                    if "image" in rel.target_ref:
                        try:
                            ...
                            context = self._find_image_context(
                                document_text, len(extracted_images)
                            )
    ...
    def _create_document_screenshot(
        self, file_path: str, source_filename: str
        ...
        try:
            ...
            draw.text(
                (width // 2, height // 2), text, fill="black", font=font, anchor="mm"
            )
    ...
    def find_relevant_images(
        ...
        is_general_image_query = any(
            keyword in query_lower for keyword in general_image_keywords
        )
